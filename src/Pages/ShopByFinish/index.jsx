...
const Index = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      if (shopByFinishModalType.isEdit) {
        handleUpdateShopByFinishData(values);
      } else {
        handleAddByShopFinish(values);
      }
    },
  });

  // function to Add shop by use data
  const handleAddByShopFinish = async (values) => {
  ...
  const handleToggleModal = () => {
    if (shopByFinishModalType.isModalOpen) {
      setShopByFinishModalType(globalConstant.InitialModalStateData);
      formik.resetForm();
  ...
  const handleGetShopByFinishData = async (queryParams) => {
    try {
      const response = await service.shopByFinishPage.listing(queryParams);
      if (response.status === 200) {
        setShopByFinishData(response?.data?.data);
      }
    } catch (error) {
      toast.error(error?.response?.data?.message, { autoClose: 2000 });
    }
  ...
  useEffect(() => {
    handleGetShopByFinishData(queryParams);
  }, [queryParams]);
  ...
  const handleEditShopByFinishData = (data) => {
    if (data) {
  ...
  const totalPages = useMemo(() => {
    return Math.ceil(shopByFinishData?.total / 10);
  }, [shopByFinishData?.total]);
    ...
    <>
      ...
      <Box>
              ...
              action: (
                ...
                >
                  <IconButton
                    aria-label="Edit"
                    color="success"
                    onClick={() => handleEditShopByFinishData(item)}
                    sx={{
                      borderRadius: "10px",
                      border: `1px solid`,
                    }}
        ...
        <AppPagination
          ...
          totalCount={shopByFinishData?.total}
      ...
      {shopByFinishModalType.isModalOpen && (
        <AppModal
          open={shopByFinishModalType.isModalOpen}
          maxWidth={"sm"}
            ...
            >
              ...
              >
                <Typography variant="title">
                  {shopByFinishModalType.isEdit
                    ? "Edit Shop By Finish Details"
                    : "Add Shop By Finish"}
                </Typography>
            ...
            <Box px={5} pb={4}>
              ...
              <Stack direction={"row"} gap={3}>
                <CustomButton
                  ...
                  buttonName="Confirm"
                  onClick={() =>
                    handleDeleteShopByFinishData(openDeleteModal.deleteId)
                  }
                />
